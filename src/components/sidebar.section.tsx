            ...
            <div className="space-y-1 w-60">
                {chats.map((chat) => (
                    ...
                    <div key={chat._id}
                        role="button"
                        tabIndex={0}
                        onClick={() => setCurrentThread(chat._id)}
                        onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") setCurrentThread(chat._id); }}
                        className={cn(
                            "flex justify-around items-center px-2 py-1 rounded-lg group hover:bg-blue-100 cursor-pointer",
                            currentThreadId === chat._id && 'bg-[#dbeaff]'
                        )}
                    >
                        ...
                        <Button
                            ...
                            onClick={(e) => { e.stopPropagation(); deleteConversation(chat._id); }}
                        >
